...
Vaf_1, Vab_1, Vzf_1, c_1 = [zeros(I,J) for i in 1:6]
...
for n = 1:maxit
    ...
    Vaf_1[I,:] = (z.*k_max.^α - δ.*k_max).^(-γ) # imposes a constraint
...
time = 1000 #periods of time for updating
...
# set up all of these empty matrices
Vaf_2, Vab_2,c_2 = [zeros(I,J) for i in 1:6]
...
for t = 1:time
    ...
    Vaf_2[I,:] = (z.*k_max.^α - δ.*k_max).^(-γ) # imposes a constraint
